...
export default function Sidebar() {
  ...
  const navLinks = [
    ...
    { name: "Endpoints", href: "/dashboard/endpoints", icon: Radio, count: endpoints.length },
    { name: "Logs", href: "/dashboard/deliveries", icon: Activity },
        ...
        <div className="bg-[#111113] border border-white/5 p-4 rounded-xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-gray-600 truncate">{email || "email"}</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="w-full py-2 cursor-pointer bg-white/5 hover:bg-red-500/10 hover:text-red-500 text-gray-400 text-[10px] font-bold uppercase rounded-lg transition-all flex items-center justify-center gap-2"
          >
            <LogOut size={12} />
            Logout
          </button>
